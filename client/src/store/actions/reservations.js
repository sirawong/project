...
export const getReservations = () => async dispatch => {
  try {
    ...
    const url = process.env.REACT_APP_BASE_URL + '/reservations/reservations';
...
//   try {
//     const token = localStorage.getItem('jwtToken');
//     const url = process.env.REACT_APP_BASE_URL + '/reservations/reservations/usermodeling/' + username;
//     const response = await fetch(url, {
//       method: 'GET',
//       headers: {
//         Authorization: `Bearer ${token}`
...
export const updateReservation = (reservation, id) => async dispatch => {
  try {
    const token = localStorage.getItem('jwtToken');
    const url = process.env.REACT_APP_BASE_URL + '/reservations/reservations/' + id;
    const response = await fetch(url, {
      method: 'PATCH',
      headers: {
        Authorization: `Bearer ${token}`,
